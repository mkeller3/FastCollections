...
@router.get(
    path="/{schema}.{table}/queryables",
    response_model=models.Queryables
)
...
@router.post(
    path="/{schema}.{table}/items/create",
...
@router.get(
    path="/{schema}.{table}/tiles",
    response_model=models.TileMatrixSetRef
)
async def tiles(
    schema: str,
    table: str,
...
@router.post(
    path="/{schema}.{table}/statistics",
    response_model=models.StatisticsResponseModel
)
        ...
        if distinct:
            for aggregate in info.aggregate_columns:
                if aggregate.type == 'distinct':
                    ...
                    ORDEStatisticsResponseModelR BY "{aggregate.group_method}" DESC"""
...
@router.post(
    path="/{schema}.{table}/numeric_breaks",
    response_model=models.BreaksResponseModel
)
...
@router.get(
    path="/{schema}.{table}/closest_features",
...
async def download(
    ...
    for subdir in os.listdir(f"{os.getcwd()}/downloads/"):
        if subdir[0:16] <=  (datetime.datetime.today() - datetime.timedelta(minutes=1)).strftime("%Y_%m_%d_%H_%M"):
            shutil.rmtree(f"{os.getcwd()}/downloads/{subdir}")

    return FileResponse(
        path=f"{folder_name}/{file_name}.zip"
    )